...
data_MI_status <- dbGetQuery(conn, "SELECT * FROM SMR01_Admissions")  #Need to filter for those with the specific code for MI in main or other condition
data_HDL_LDL_1 <- dbGetQuery(conn, "SELECT * FROM GD_StudyDataGD2")
data_HDL_LDL_2 <- dbGetQuery(conn, "SELECT * FROM GD_StudyDataGD3")
data_age <- dbGetQuery(conn, "SELECT * FROM Demography_Current")

#Cleaning data to keep relevant columns:

# MI df now contains only rows where main condition isI2190
data_MI_status <- data_MI_status %>%
  filter(MAIN_CONDITION == "I2190")

#Create one data frame containing all the important information:

#First keep only the important columns of all the dfs

data_age_filtered <- data_age[,c(1,2,7)] #PROCHI, SEX, AGE

data_HDL_LDL_1_filtered <- data_HDL_LDL_1[, c("PROCHI","Present_Smoker","Past_Smoker","HDL","LDL")]
data_HDL_LDL_2_filtered <- data_HDL_LDL_2[, c("PROCHI","Present_Smoker","Past_Smoker","HDL","LDL")]

data_MI_status_filtered <- data_MI_status[,c("PROCHI","MAIN_CONDITION")]
rs3764261_filtered <- rs3764261[,c(1,7)]
rs629301_filtered <- rs629301[,c(1,7)]

colnames(rs629301_filtered)[1] <- "PROCHI"
colnames(rs3764261_filtered)[1] <- "PROCHI"

#First make one df with the hdl and ldl data
Data_HDL_LDL_final <- rbind(data_HDL_LDL_1_filtered, data_HDL_LDL_2_filtered)


#Lets make a table like the one for the simulated data

# List of data frames
df_list <- list(data_age_filtered, Data_HDL_LDL_final, rs3764261_filtered, rs629301_filtered)

# Merge all data frames by the column "PROCHI"
merged_df <- Reduce(function(x, y) merge(x, y, by = "PROCHI"), df_list)

#Check missing values in our df:
missing_summary <- sapply(merged_df, function(x) sum(is.na(x)))
print(missing_summary)

merged_df$MI_Status <- ifelse(merged_df$PROCHI %in% data_MI_status$PROCHI, TRUE, FALSE)

sum(merged_df$Present_Smoker == 'N' & merged_df$Past_Smoker == 'N' , na.rm = TRUE)

#Convert smoking into a column with 1 and 0, if they have ever smoked they are considered a smoker (1)
merged_df$Smoking_status <- ifelse(merged_df$Present_Smoker == "Y" | merged_df$Past_Smoker == "Y", 1, 0)

#Convert Sex into 1(Male) and 0 (Female)
merged_df$sex <- ifelse(merged_df$sex == "M", 1, 0)

#Remove missing values for HDL and LDL and smoking status
merged_df_final <- na.omit(merged_df)
merged_df_final <- merged_df_final[ , -c(4,5)]

#Remove Duplicates
merged_df_final <- merged_df_final[!duplicated(merged_df_final$PROCHI), ]


#Now we can begin our analysis
colnames <- c("PROCHI","sex","age","hdl","ldl","snp.ldl","snp.hdl","mi","smoking")
colnames(merged_df_final) <- colnames

#Final thing we need is the follow up days for each individual in our final data frame

#Add columns required to calculate follow up time (participant_data, Admission Data for MI, hix_validation time from demography data for last observed/followed up time)
merged_df_final <- merge(merged_df_final, data_HDL_LDL_1[,c("PROCHI","Participant_Date")], by = "PROCHI", all.x = TRUE)
merged_df_final <- merge(merged_df_final, data_MI_status[,c("PROCHI","ADMISSION_DATE")], by = "PROCHI", all.x = TRUE)
merged_df_final <- merge(merged_df_final, data_age[,c("PROCHI","hic_validFrom")],by = "PROCHI", all.x = TRUE )

table(is.na(merged_df_final$Participant_Date) == TRUE & is.na(merged_df_final$ADMISSION_DATE) ==TRUE)


#Remove the 3 individuals whom we have no data for regarding participant data
merged_df_final <- merged_df_final[complete.cases(merged_df_final$Participant_Date),]

#Calculate the follow.up.days
# Create a new column based on the specified criteria
merged_df_final$follow.up.days <- ifelse(is.na(merged_df_final$ADMISSION_DATE), as.numeric(difftime(merged_df_final$hic_validFrom, merged_df_final$Participant_Date, units = "days")), as.numeric(difftime(merged_df_final$ADMISSION_DATE, merged_df_final$Participant_Date, units = "days")))



mi.data <- merged_df_final[,c(1:9,13)]
mi.data$follow.up.days <- floor(as.numeric(mi.data$follow.up.days))
...
p1 <- ggplot(mi.data, aes(x = mi , y = hdl, fill = as.factor(mi))) +
  ...
  stat_compare_means(label.y = c(2.5, 2.5), method = "t.test", label = "p.format", vjust = -1) +  # Add p-value
  theme_bw() +
...
p2 <- ggplot(mi.data, aes(x = smoking , y = hdl, fill = as.factor(smoking))) +
  ...
  stat_compare_means(label.y = c(0.5, 2), method = "t.test", label = "p.format", vjust = -1) +  # Add p-value
  theme_bw() +
...
p3 <- ggplot(mi.data, aes(x = sex , y = hdl, fill = as.factor(sex))) +
  ...
  stat_compare_means(label.y = c(0.5, 2.3), method = "t.test", label = "p.format", vjust = -1) +  # Add p-value
  theme_bw() +
...
p4 <- ggplot(mi.data, aes(x = age , y = hdl)) +
  geom_density_2d() + # Add density cloud
  geom_point(alpha = 0)+
...
p1_ldl <- ggplot(mi.data, aes(x = mi , y = ldl, fill = as.factor(mi))) +
  geom_boxplot(outlier.shape = NA) +
  scale_fill_manual(values = c("dodgerblue","red"), name = "MI") +
  stat_compare_means(label.y = c(3, 4), method = "t.test", label = "p.format", vjust = -1) +  # Add p-value
  theme_bw() +
...
p2_ldl <- ggplot(mi.data, aes(x = smoking , y = ldl, fill = as.factor(smoking))) +
  geom_boxplot(outlier.shape = NA) +
  scale_fill_manual(values = c("dodgerblue","red"), name = "Smoker") +
  stat_compare_means(label.y = c(0.5, 3), method = "t.test", label = "p.format", vjust = -1) +  # Add p-value
  theme_bw() +
